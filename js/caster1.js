...
db.ref("/live-graphics/caster1/casterName").once("value", function(snap) {
  if (!snap.val()) {
...
document.getElementById("name-submit").onclick = function() {
  ...
  if (name) {
    db.ref("/live-graphics/caster1/casterName").set(name);
...
function saveChoices() {
  ...
  db.ref("/live-graphics/caster1/" + currentMatch).set(teams.length ? teams : null);
...
function resetChoices() {
  ...
  db.ref("/live-graphics/caster1/" + currentMatch).remove();
...
db.ref("/live-graphics/caster1").on("value", function(snap) {
  var val = snap.val() || {};
  for (var m in val) {
